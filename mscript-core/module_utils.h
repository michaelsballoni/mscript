...
namespace mscript
{
	...
	{
	public:
		inline static wchar_t* cloneString(const wchar_t* str)
		{
		...
		inline static wchar_t* getExports(const std::vector<std::wstring>& exports)
		{
			return cloneString(join(exports, L", ").c_str());
		}

		inline static wchar_t* jsonStr(const object& obj)
		{
			const std::wstring json = objectToJson(obj);
			return cloneString(json.c_str());
		}

		inline static wchar_t* jsonStr(const std::string& str)
		{
			return jsonStr(toWideStr(str));
		}

		inline static wchar_t* errorStr(const std::wstring& function, const std::exception& exp)
		{
